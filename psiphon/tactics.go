...
func GetTactics(ctx context.Context, config *Config, useStoredTactics bool) (fetchedTactics bool) {
	...
	if useStoredTactics {
		...
		tacticsRecord, err = tactics.UseStoredTactics(
			...
			config.GetNetworkID())
		if err != nil {
			NoticeWarningf("get stored tactics failed: %s", errors.Trace(err))
	...
	if tacticsRecord == nil {
		...
		if err != nil {
			NoticeWarningf("tactics iterator failed: %s", errors.Trace(err))
			return
		}
		...
		for iteration := 0; ; iteration++ {
			...
			NoticeWarningf("tactics request failed: %s", errors.Trace(err))
	...
	if tacticsRecord != nil {
		...
		if err != nil {
			NoticeWarningf("apply tactics failed: %s", errors.Trace(err))
